...
export default function RecurringPayment() {
	...
	const [payments, setPayments] = useState<{ id: string, label: string, amount: string, date: string }[]>([])
			...
			>
				...
				{!showForm ? (
					...
					<View className="w-full max-w-md bg-white rounded-2xl p-5 shadow-md mb-8 self-center">
						...
						>
							<Text className={`text-base ${recurringDate ? 'text-gray-800' : 'text-gray-400'}`}>
								{recurringDate
					...
					<FlatList
						...
						renderItem={({ item }) => (
							<View className="flex-row justify-between items-center bg-white px-4 py-3 mb-3 rounded-xl shadow-sm border border-gray-200">
								...
								<Text className="text-base text-gray-700">₦{item.amount}</Text>
							</View>
						)}
					/>
				) : (
