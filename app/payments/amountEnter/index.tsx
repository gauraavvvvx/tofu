...
const UPIPaymentScreen = () => {
  ...
  const categories = ['Groceries', 'Utilities', 'Entertainment', 'Dining', 'Others'];
  ...
  useEffect(() => {
    ...
    if (params?.upiUri) {
      const upiUri = Array.isArray(params.upiUri) ? params.upiUri[0] : params.upiUri;
      parseUPIURI(upiUri);
    }
      ...
      {amount && roundedAmount > 0 && (
        <View style={styles.savingsContainer}>
          <Text style={styles.savingsText}>
            You are paying <Text style={styles.highlight}>₹{roundedAmount}</Text> and saving <Text style={styles.highlight}>₹{(roundedAmount - parseFloat(amount)).toFixed(2)}</Text>!
          </Text>
        </View>
      )}
